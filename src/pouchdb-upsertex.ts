...
type DiffFunc = (doc: any) => Promise<Boolean|any>|Boolean|any;
...
async function upsertInner(db: PouchDB.Database, docId: string, diffFun: DiffFunc, options?, count?: number) {
  ...
  const newDoc = await diffFun(doc || {});
...
async function tryAndPut(db: PouchDB.Database, doc, diffFun, options= {}, count: number = MAX_RETRY) {
  let res;
  try {
...
export async function upsertEx(this: PouchDB.Database, docId, diffFun, options?) {
  const db = this;
  const count = (options && options.retry) || MAX_RETRY;
  return upsertInner(db, docId, diffFun, options, count);
}

export async function upsert(this: PouchDB.Database, aDoc, options?) {
  if (aDoc._id === undefined) throwError('Missing _id', 'post', 404);
  const db = this;
  const count = (options && options.retry) || MAX_RETRY;
  return upsertInner(db, aDoc._id, () => aDoc, options, count);
}

export async function putIfNotExists(this: PouchDB.Database, docId, doc?, options?) {
  const db = this;
...
export async function postEx(this: PouchDB.Database, aDoc, aOptions?) {
  if (aDoc._id === undefined) throwError('Missing _id', 'post', 404);
